// GET /api/submissions/:id/email      → stream HTML email
// GET /api/submissions/:id/email-txt  → handled by email-txt.ts sibling
...
export async function onRequestGet({ request, env, params }: any) {
  ...
  const row = await env.submoacontent_db
    .prepare("SELECT html_r2_key FROM email_submissions WHERE submission_id = ?")
    .bind(id)
    .first<{ html_r2_key: string | null }>();
  if (!row?.html_r2_key) return new Response("Email not yet built", { status: 404 });

  const obj = await env.SUBMOA_IMAGES.get(row.html_r2_key);
